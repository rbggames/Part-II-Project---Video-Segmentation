...
int TrackedObjects::find(Mat frame)
{
				...
				if (objectBoundingBoxes[j].area() > 100) {
					...
					for (int objectId = 0; objectId < trackedObjectList.size(); objectId++) {
						...
						if (((expandedBoundingBox & objectBoundingBox).area() > 0)) {
							...
							if (Utilities::isAngleBetween(angle, (i * bin_size - bin_size*0.75), (i * bin_size + bin_size*1.75))) {
	...
	/// Display
	imshow("Histogram of Angles", histImage);
	cv::cvtColor(frame, prevFrame, CV_BGR2GRAY);
...
int TrackedObjects::update(Mat frame, Mat outputFrame) {
