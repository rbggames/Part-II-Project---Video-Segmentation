...
ShapeHook::ShapeHook(int x, int y,int width,Scalar colour)
{
	...
	Rect2d boundingBoxes[10];

	Mat temp;
	shapeMat.copyTo(temp);

	int num = Utilities::find_boundingBoxes(temp, boundingBoxes, 1);
	if (num > 0) {
		xOffset = boundingBoxes[0].tl().x;
		yOffset = boundingBoxes[0].tl().y;
	}
	else {
		xOffset = 0;
		yOffset = 0;
