...
{
	//Update tracker
	bool skipMotionEstimation = false;
	bool ok = false;
	if (tracker) {
		if (!predicting)
			...
			trackerBoundingBox.x += motionVector.val[0];
			trackerBoundingBox.y += motionVector.val[1];
			return true;
		}
	}

	if (!predicting && trackingReset) {
		imshow("ASDF", object);
		imshow("adf", frame);
		tracker->clear();
		tracker->empty();
		...
		tracker = TrackerKCF::create();
		//trackerBoundingBox = Rect2d(objectBoundingBox);
		tracker->init(frame, trackerBoundingBox);
		trackerBoundingBox.x = 100;
		ok = tracker->update(frame, trackerBoundingBox);
		skipMotionEstimation = true;
		trackingReset = false;

		/*Ptr<Tracker> t = TrackerKCF::create();
		Mat* curFrame = new Mat(frame);
		frame.copyTo(*curFrame);
		t->init(*curFrame, objectBoundingBox);
		trackerBoundingBox = Rect2d(objectBoundingBox);
		ok = t->update(*curFrame, trackerBoundingBox);*/
		//free(curFrame);
	}
	double alpha = 0.1;
	...
	if (ok) {
		...
		if (!skipMotionEstimation) {
			if (!trackingReset  && motionVector.val[0] != 0) {
				motionVector.val[0] = alpha*((float)trackerPosition.x - previousPosition[positionIndex].x) + (1 - alpha)*motionVector.val[0];
			}
			else {
				//Initialise
				if (motionVector.val[0] == 0)
					motionVector.val[0] = ((float)trackerPosition.x - previousPosition[positionIndex].x);
			}

			if (!trackingReset && motionVector.val[0] != 0) {
				motionVector.val[1] = alpha*((float)trackerPosition.y - previousPosition[positionIndex].y) + (1 - alpha)*motionVector.val[1];
			}
			else {
				//Initialise
				motionVector.val[1] = ((float)trackerPosition.y - previousPosition[positionIndex].y);
				trackingReset = false;
			}
		}
...
void TrackedObject::drawSegment(Mat frame,bool isOverlap, Mat outputFrame)
{
	...
	if (!isOverlap) {//TODO:CHANGE
		...
		rectangle(object, objectBoundingBox, Scalar(0, 255, 255), 3);
		imshow("sav", savedObject);
		Point2f src[3] = { Point2f(0, 0), Point2f(0,savedObject.rows - 1),Point2f(savedObject.cols - 1,savedObject.rows - 1) };
		Point2f dst[3] = { Point2f(objectBoundingBox.x, objectBoundingBox.y),
			Point2f(objectBoundingBox.x, savedObject.rows - 1 + objectBoundingBox.y),
			Point2f(savedObject.cols - 1 + objectBoundingBox.x, savedObject.rows - 1 + objectBoundingBox.y) };
...
Vec2d TrackedObject::getPosition()
{
	if (predicting)
		return Vec2d(position.x,position.y);
	else {
		Rect2d boundingBoxes[1000];
		Mat tempMask;
		cvtColor(mask, tempMask, CV_GRAY2BGR);
		Utilities::find_boundingBoxes(tempMask, boundingBoxes, 1);
		int xPos = boundingBoxes[0].tl().x;
		int yPos = boundingBoxes[0].tl().y;
...
Mat TrackedObject::getOnlyObjectMat()
{
	Mat obj;
	object.copyTo(obj);
	obj.setTo(Scalar(0, 0, 0));
	int height = objectBoundingBox.height;
	int width = objectBoundingBox.width;
	Point2f src[3] = { Point2f(0, 0), Point2f(0,savedObject.rows - 1),Point2f(savedObject.cols - 1,savedObject.rows - 1) };
	...
	if(savedObject.size().area() > 0)
		warpAffine(savedObject, obj, translationMat, object.size(), 1, BORDER_TRANSPARENT);
